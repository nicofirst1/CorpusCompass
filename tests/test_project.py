...
class ProjectTestCase(unittest.TestCase):
    def test_adding_ivs_and_iv_values(self):
        ...
        def error_occured_received(message: str):
            ...
            self.assertEqual(message, 'The Independent Variable with the name "Non-existing Variable" was not found!')
    ...
    def test_adding_dv_values(self):
        ...
        def error_occured_received(message: str):
            error_received[0] = True
            self.assertEqual(message, 'The dependent variable value "GER_Word" already exists!')
    ...
    def test_assigning_dv_values_to_dv(self):
        ...
        self.assertEqual(len(project.dependent_variables["Language"].get_variable_values()), 3)

        project.add_dv("Formality", ["INFORMAL", "FORMAL"])
        self.assertEqual(len(project.dependent_variables["Formality"].get_variable_values()), 2)

        # Assigning a dv value that does not exist should not be possible
        error_received = [False]
        def error_occured_received(message: str):
            error_received[0] = True
            self.assertEqual(message, 'The dependent variable value "Non-existing Value" does not exist!')
