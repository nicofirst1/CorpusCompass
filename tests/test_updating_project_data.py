...
class UpdateProjectDataTestCase(unittest.TestCase):
    ...
    def test_updating_iv(self):
        ...
        project.update_iv_from_frontend(
            {"Age": ["Old", "Young", "Middle"], "Language": ["German", "English"]}
        )

        # Check if the IVs have been updated correctly
        self.assertEqual(
            project.get_iv_printable(),
            {"Age": ["Old", "Young", "Middle"], "Language": ["German", "English"]},
        )
    ...
    def test_updating_dv(self):
        ...
        project.update_dv_values_from_frontend(
            {"Informal": (2, "Formality", "red"), "Formal": (1, "Formality", "blue")}
        )

        # Check if the DVs have been updated correctly
        self.assertEqual(
            project.get_dv_printable(),
            {"Formality": (["Informal", "Formal"], ["red", "blue"])},
        )
    ...
    def test_updating_speakers(self):
        ...
        project.update_speakers_from_frontend(
            {
                "Speaker A": ({"Age": "Old"}, "red"),
                "Speaker B": ({"Age": "Young"}, "green"),
            }
        )

        # Check if the Speakers have been updated correctly
        self.assertEqual(
            project.get_speakers_printable(),
            {
                "Speaker A": ({"Age": "Old"}, "red"),
                "Speaker B": ({"Age": "Young"}, "green"),
            },
    ...
    def test_updating_annotation_formats(self):
        ...
        self.assertEqual(
            project.get_annotation_formats_printable(), {"[$token.identifier]": ["_"]}
        )
