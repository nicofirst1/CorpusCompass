...
class SpeakerDetectorTestCase(unittest.TestCase):
    ...
    def test_finding_speakers_in_files(self):
        ...
        f1 = File("File1", "utf8", "./file1.txt", 1.0, "Speaker A: Hey, how are you? Speaker B: I'm fine thanks, what about you? Speaker C: I'm good, thank you.")
        f2 = File("File2", "utf8", "./file2.txt", 2.0, "Speaker C: Have you already learned for the exam next week? Speaker D: Nah, I will rely on my luck.")

        detected_speakers = speaker_detector.detect_speakers([f1, f2])

        self.assertEqual(list(detected_speakers["speaker_name"].unique()), ["Speaker A", "Speaker B", "Speaker C", "Speaker D"])
...
class AnnotationDetectorTestCase(unittest.TestCase):
    def test_creating_re_from_str(self):
        ...
        annotation_detector.add_annotation_format_token_identifier(annotation_str=annotation_format_str,
                                                                   token=(2, 6),
                                                                   identifier=(8, 17))
        annotation_re, _ = annotation_detector.get_annotation_format(annotation_format_str)
    ...
    def test_finding_annotations_in_files(self):
        ...
        """
        annotation_detector = AnnotationDetector()
        annotation_detector.add_annotation_format_token_identifier(annotation_str="[$TOKEN.IDENTIFIER]",
                                                                   token=(2, 6),
                                                                   identifier=(8, 17))
        
        f1 = File("File1", "utf8", "./file1.txt", 1.0, "Speaker A: Wow, those new shoes look so [$cool.INFORMAL]. Speaker B: Thank you, I bought them from [$germany.GER].")
        f2 = File("File2", "utf8", "./file2.txt", 2.0, "Speaker C: I have a lot of [$work.JOB] to do.")
    ...
    def test_finding_multiple_identifiers(self):
        ...
        f1 = File("File1", "utf8", "./file1.txt", 1.0, "Speaker A: Winter is [$coming.IDENTIFIER1_IDENTIFIER2]")
        f2 = File("File2", "utf8", "./file2.txt", 2.0, "Speaker B: I have a lot of [$work.IDENTIFIER1_IDENTIFIER2_IDENTIFIER3_IDENTIFIER4] to do.")

        detected_annotations = annotation_detector.detect_annotations([f1, f2])
        self.assertEqual(detected_annotations.loc[0, "identifier"], ["IDENTIFIER1", "IDENTIFIER2"])
        self.assertEqual(detected_annotations.loc[1, "identifier"], ["IDENTIFIER1", "IDENTIFIER2", "IDENTIFIER3", "IDENTIFIER4"])

    def test_finding_annotations_with_identifier_before_token(self):
        """Test if the annotation detector can find multiple identifiers in an annotation
        """
        annotation_detector = AnnotationDetector()
        annotation_detector.add_annotation_format_token_identifier(annotation_str="[$IDENTIFIER.TOKEN]",
                                                                   token="TOKEN",
                                                                   identifier="IDENTIFIER",
                                                                   multiple_identifier_separator=".")
        
        file = File("File1", "utf8", "./file1.txt", 1.0, "Speaker A: Winter is [$IDENTIFIER1.IDENTIFIER2.coming]")
