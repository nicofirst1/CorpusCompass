...
class LoadFiles(GeneralWindow):
    ...
    def create_widgets(self):
        ...
        self.postprocess_button = QtWidgets.QPushButton(
            "(Optional) Load Pos-Processed data"
        )
    ...
    def loading_message(self):
        ...
        variables = (
            ...
            + "\n\t--".join(self.variables_files)
            + "\n"
            if self.variables_files
            else "Variables file not uploaded"
        )
    ...
    def load_postporcess_files(self):
        ...
        dir = QFileDialog.getExistingDirectory(self, "Select Annotation Folder", dir)
        ...
        if len(found) != len(self.mem.postprocess_names):
            ...
            missing = [
                f
                for f in self.mem.postprocess_names
                if f not in [os.path.basename(x) for x in files]
            ]
            err_msg = (
                f"Error loading post-process files:\nCould not find the following files:\n\t--"
                + "\n\t--".join(missing)
                + "\n Please try again"
            )
            self.corpus_message.setText(err_msg)
    ...
    def load_variables(self):
        ...
        if dir:
            ...
            # check that all files in self.mem.postprocess_names are in the folder
            found = [
                f for f in files if os.path.basename(f) in self.mem.variables_names
            ]
            ...
            if len(found) != len(self.mem.variables_names):
                ...
                missing = [
                    f
                    for f in self.mem.variables_names
                    if f not in [os.path.basename(x) for x in files]
                ]
                err_msg = (
                    f"Error loading variables files:\nCould not find the following files:\n\t--"
                    + "\n\t--".join(missing)
                    + "\n Please try again"
                )
                self.corpus_message.setText(err_msg)
