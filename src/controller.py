...
from PySide6.QtWidgets import QListWidgetItem, QDialog, QLabel, QTreeWidgetItem, QRadioButton, QFileDialog
...
class Controller(QObject):
    ...
    def __init__(self, model: CorpusCompassModel, view: CorpusCompassView) -> None:
        ...
        start_screen_tab.create_new_project.clicked.connect(self.on_create_project_clicked)
        ...
        home_menu_tab.btn_home_speaker_sect.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.SPEAKER_TAB))
        home_menu_tab.btn_home_annotformat_sect.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.ANNOTATIONFORMAT_TAB))
        home_menu_tab.btn_home_managecorpus_sect.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.LOAD_FILES_TAB))
        home_menu_tab.btn_home_varmanag_sect.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.VARMANAGEMENT_TAB))
        home_menu_tab.btn_home_analysissettings_sect.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.ANALYSIS_SETTINGS_TAB))
        home_menu_tab.btn_settings.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.GENERAL_SETTINGS_TAB))
        home_menu_tab.btn_closeproject.clicked.connect(self.on_project_closed_clicked)

        home_menu_tab.btn_projectinformation.clicked.connect(self.on_projectinformation_clicked)
        home_menu_tab.btn_home_analysecorpus.clicked.connect(self.on_analyse_corpus_button_clicked)

        # General-Settings-Tab
        general_settings_tab = self.view.general_settings_tab
        general_settings_tab.btn_back.clicked.connect(lambda: self.on_home_section_button_clicked(Tab.HOME_TAB))

        # Speaker-Identification-Section
        speaker_section_tab = self.view.speaker_tab
        speaker_section_tab.btn_sp_save.clicked.connect(self.on_speaker_format_saved_clicked)
        speaker_section_tab.btn_sp_cancel.clicked.connect(self.on_speaker_cancel_button_clicked)

        speaker_section_tab.btn_expand_speakerinfo.clicked.connect(self.view.speaker_tab.on_speaker_expand_clicked)
        speaker_section_tab.btn_expand_wordinfo.clicked.connect(self.view.speaker_tab.on_words_expand_clicked)

        speaker_section_tab.radbtn_sp_standard.toggled.connect(lambda: self.on_speaker_format_changed(speaker_section_tab.radbtn_sp_standard))
        speaker_section_tab.radbtn_sp_praat.toggled.connect(lambda: self.on_speaker_format_changed(speaker_section_tab.radbtn_sp_praat))
        speaker_section_tab.radbtn_sp_flex.toggled.connect(lambda: self.on_speaker_format_changed(speaker_section_tab.radbtn_sp_flex))
        speaker_section_tab.radbtn_sp_elan.toggled.connect(lambda: self.on_speaker_format_changed(speaker_section_tab.radbtn_sp_elan))

        # Annotation-Format-Section
        annot_format_tab = self.view.annotation_format_tab
        annot_format_tab.btn_annotformat_save.clicked.connect(self.on_annotformat_section_saved_clicked)
        # annot_format_tab.btn_annotformat_removerows.clicked.connect(self.on_annotformat_section_removeformats_clicked)
        annot_format_tab.btn_help.clicked.connect(self.on_annotformat_help_button_clicked)
        annot_format_tab.btn_annotformat_cancel.clicked.connect(self.on_annotformat_cancel_button_clicked)
        annot_format_tab.btn_annotformat_addnew.clicked.connect(lambda: self.on_open_annotation_format_editor(row=-1, column=-1, called_for_add=True))
        annot_format_tab.btn_annotformat_edit.clicked.connect(lambda: self.on_open_annotation_format_editor(row=-1, column=-1, called_for_add=False))
        annot_format_tab.tableWidget_annotformats.cellDoubleClicked.connect(self.on_open_annotation_format_editor) # Call without lambda to send row and column automatically (thats why called_for_add is default False, which is what the function will be called with)

         # Load File Tab
        load_files_tab = self.view.load_files_tab
        load_files_tab.btn_add_file.clicked.connect(self.on_add_file_clicked)
        load_files_tab.btn_remove_file.clicked.connect(self.on_remove_selected_files_clicked)
        load_files_tab.list_loaded_filenames.cellDoubleClicked.connect(self.on_file_double_clicked)
        ...
        variable_management_tab.btn_save_changes.clicked.connect(self.on_metadata_saved_clicked)
        ...
        variable_management_tab.btn_add_iv.clicked.connect(lambda: self.on_open_metadata_iv_editor(item=None, column=0, called_for_add=True))
        variable_management_tab.btn_edit_ivs.clicked.connect(lambda: self.on_open_metadata_iv_editor(item=None, column=0, called_for_add=False)) # If not double-clicked, then explicitely send item=None to ensure that no item is selected in the dialog initially
        variable_management_tab.treeWidget_ivs.itemDoubleClicked.connect(self.on_open_metadata_iv_editor) # Call without lambda to send item automatically via the signal (called_for_add is default False, which is what the function will be called with the argument is not further specified here)

        variable_management_tab.btn_add_dv.clicked.connect(lambda: self.on_open_metadata_dv_editor(item=None, column=0, called_for_add=True))
        variable_management_tab.btn_edit_dvs.clicked.connect(lambda: self.on_open_metadata_dv_editor(item=None, column=0, called_for_add=False)) # If not double-clicked, then explicitely send item=None to ensure that no item is selected in the dialog initially
        variable_management_tab.treeWidget_dvs.itemDoubleClicked.connect(self.on_open_metadata_dv_editor) # Call without lambda to send item automatically via the signal (called_for_add is default False, which is what the function will be called with the argument is not further specified here)

        variable_management_tab.btn_add_speaker.clicked.connect(lambda: self.on_open_metadata_speaker_editor(item=None, column=0, called_for_add=True))
        variable_management_tab.btn_edit_speakers.clicked.connect(lambda: self.on_open_metadata_speaker_editor(item=None, column=0, called_for_add=False)) # If not double-clicked, then explicitely send item=None to ensure that no item is selected in the dialog initially
        variable_management_tab.treeWidget_speakers.itemDoubleClicked.connect(self.on_open_metadata_speaker_editor) # Call without lambda to send item automatically via the signal (called_for_add is default False, which is what the function will be called with the argument is not further specified here)

        variable_management_tab.btn_import_metadata.clicked.connect(self.on_import_metadata_button_clicked)
        variable_management_tab.btn_export_metadata.clicked.connect(self.on_export_metadata_button_clicked)

        variable_management_tab.btn_detectvariants.clicked.connect(self.on_detect_variants_clicked)

        # Analysis Settings Tab
        analysis_settings_tab = self.view.analysis_settings_tab
        analysis_settings_tab.btn_close_overview.clicked.connect(self.on_close_analysis_overview_button_clicked)
        analysis_settings_tab.btn_save_settings.clicked.connect(self.on_analysis_settings_save_button_clicked)

        analysis_settings_tab.checkBox_showannotations.toggled.connect(self.on_highlight_dvs_clicked)
    ...
    def on_highlight_dvs_clicked(self, toggled: bool):
        ...
        self.view.analysis_settings_tab.highlight_dvs(toggled, dv_values, self.model.current_project.get_detected_annotations()) 
    ...
    def on_underline_speakers_clicked(self, toggled: bool):
        ...
        self.view.analysis_settings_tab.underline_speakers(toggled, speakers, self.model.current_project.get_detected_speakers()) 
    ...
    def on_create_project_clicked(self) -> None:
        ...
        self.view.create_proj_dialog.create_proj_accepted.connect(self.on_create_project_accepted)
    ...
    def connect_signals_to_project(self):
        ...
        self.model.current_project.iv_changed_signal.connect(self.on_metadata_iv_changed)
        self.model.current_project.dv_changed_signal.connect(self.on_metadata_dv_changed)
        self.model.current_project.dv_values_changed_signal.connect(self.on_metadata_dv_values_changed)
        self.model.current_project.speaker_changed_signal.connect(self.on_metadata_speaker_changed)

        self.model.current_project.annotation_formats_changed_signal.connect(self.on_annotation_formats_changed)
        self.model.current_project.speaker_preview_signal.connect(self.on_speaker_preview_changed)
        self.model.current_project.annotation_preview_signal.connect(self.on_annotation_preview_changed)

        self.model.current_project.corpus_analysis_data_signal.connect(self.on_analysis_finished)
    ...
    def on_open_project_clicked(self):
        ...
        self.view.open_project_dialog.btn_changepath.clicked.connect(self.view.open_project_dialog.on_change_path_clicked)
    ...
    def on_home_section_button_clicked(self, target_tab: Tab):
        ...
        if target_tab == Tab.SPEAKER_TAB: # Update the speaker preview, if user goes into speaker tab
...
# Buttons in sections
    ...
    def on_speaker_format_saved_clicked(self):
        ...
        self.model.current_project.detect_speakers(is_synchronous=False, save_detected_speakers=True)
        self.view.switch_to_tab(Tab.HOME_TAB)
    ...
    def on_speaker_cancel_button_clicked(self):
        ...
        if dialog_result == QDialog.DialogCode.Accepted:
            self.view.switch_to_tab(Tab.HOME_TAB) # Only switch to Home-Tab if "Save settings" is clicked        
    ...
    def on_annotformat_section_saved_clicked(self):
        ...
        self.on_annotation_formats_changed(self.model.current_project.get_annotation_formats_printable())

        self.view.create_detect_variants_dialog(detected_variants=self.model.current_project.get_dv_values_printable())
    ...
    def on_annotationformat_remove_clicked(self):
        ...
        if self.view.annotationformat_editor_dialog.dont_show_removaldialog == False:
            ...
            self.view.annotation_removal_dialog.remove_format_accepted.connect(self.on_annotformat_section_removeformat_accepted)
    ...
    def on_annotformat_section_removeformat_accepted(self, dont_show_again: bool):
        ...
        """
        self.view.annotationformat_editor_dialog.dont_show_removaldialog = dont_show_again
    ...
    def on_open_annotation_format_editor(self, row: int, column: int, called_for_add: bool = False):
        """
        ...
        if called_for_add:
            ...
            self.view.create_annotationformat_editor_dialog(row, column, symbols, formats)
            ...
            self.view.annotationformat_editor_dialog.btn_delete_annotformat.clicked.connect(self.on_annotationformat_remove_clicked)
            self.view.annotationformat_editor_dialog.comboBox_selectformat.currentIndexChanged.connect(self.view.annotationformat_editor_dialog.on_combobox_activated)

        # Connect signals for both cases (adding and editing) to the same functions, as the functions are the same for both cases
        self.view.annotationformat_editor_dialog.btn_annotdial_resetformat.clicked.connect(self.view.annotationformat_editor_dialog.clear_input_format) # Connect signal directly to view if only view-changes are made/maybe also all in the view
        self.view.annotationformat_editor_dialog.btn_resetseparator.clicked.connect(self.view.annotationformat_editor_dialog.clear_separator_symbols)
        self.view.annotationformat_editor_dialog.btn_annotdial_addsymbols.clicked.connect(self.on_annotformat_addsymbols_clicked)
        self.view.annotationformat_editor_dialog.btn_testcurrentformat.clicked.connect(self.on_test_annotation_format_clicked)

        self.view.annotationformat_editor_dialog.btn_save.clicked.connect(self.on_save_annotation_format_clicked)
        ...
        self.view.annotationformat_editor_dialog.listwidget_annotformat_removeelems.itemChanged.connect(self.view.annotationformat_editor_dialog.clear_trashcan_symbols)

        self.view.annotationformat_editor_dialog.checkbox_annotdial_multipleannot.toggled.connect(self.view.annotationformat_editor_dialog.on_multiple_annot_checked)
    ...
    def on_test_annotation_format_clicked(self):
        ...
        current_format_as_string = self.view.annotationformat_editor_dialog.get_current_format_as_string()
        self.model.current_project.get_annotation_preview(current_format_as_string, is_synchronous=False)
    ...
    def on_annotation_preview_changed(self, preview_data: pd.DataFrame) -> None:
        ...
        tuple_list: List[Tuple[str, str, int]] = list(zip(tokens, identifiers, identifier_length))
    ...
    def on_save_annotation_format_clicked(self):
        ...
        is_duplicate = self.view.annotationformat_editor_dialog.update_temporary_dialog_model()
        current_formats = self.view.annotationformat_editor_dialog.get_updated_data()
        if self.view.annotationformat_editor_dialog.check_for_valid_formats() and not is_duplicate: # --> If the format is valid, the dialog is closed
            # Close dialog
            self.view.annotationformat_editor_dialog.accept()
            # Update annotation formats in the model
            self.model.current_project.update_annotation_formats_from_frontend(current_formats)
    ...
    def on_annotformat_addsymbols_clicked(self):
        ...
        self.view.add_symbol_dialog.btn_addsymbol.clicked.connect(self.on_add_symbol_to_annotsymbols_clicked)
        ...
        if dialog_result == QDialog.DialogCode.Accepted:
            ...
            while self.view.add_symbol_dialog.listwidget_annotformat_symbolcontainer.count() > 0:
                new_symbols.append(self.view.add_symbol_dialog.listwidget_annotformat_symbolcontainer.takeItem(0)) 
    ...
    def on_open_metadata_iv_editor(self, item: QTreeWidgetItem, column, called_for_add: bool = False):
        """
        ...
        if called_for_add:
            ...
            self.view.create_edit_iv_dialog(item, column=column, current_variables=model_ivs)

            # connect signals that are only needed if the dialog is opened for editing existing IVs
            self.view.iv_editor_dialog.comboBox_selectiv.currentIndexChanged.connect(self.view.iv_editor_dialog.on_combobox_activated)
            self.view.iv_editor_dialog.btn_editconfirm.clicked.connect(self.view.iv_editor_dialog.on_input_change_confirmed)
            self.view.iv_editor_dialog.btn_delete_iv.clicked.connect(self.on_iv_delete_button_clicked)

        # connect signals that are needed in both cases (adding and editing)
        self.view.iv_editor_dialog.btn_removevalue.clicked.connect(self.on_iv_values_remove_clicked)
        self.view.iv_editor_dialog.btn_addvalue.clicked.connect(self.on_add_iv_value_button_clicked)
        self.view.iv_editor_dialog.lineEdit_nameinput.textChanged.connect(self.view.iv_editor_dialog.check_varname_changed)
    ...
    def on_open_metadata_dv_editor(self, item: QTreeWidgetItem, column, called_for_add: bool = False):
        """
        ...
        if called_for_add:
            self.view.create_add_dv_dialog(detected_variants=detected_variants, current_variables=current_variables)
        else:
            ...
            if item is not None:
                item = self.get_tree_item_parent(item)
            self.view.create_edit_dv_dialog(clicked_item=item, column=column, detected_variants=detected_variants, current_variables=current_variables)

            # connect signals that are only needed if the dialog is opened for editing existing DVs
            self.view.dv_editor_dialog.comboBox_selectdv.currentIndexChanged.connect(self.view.dv_editor_dialog.on_combobox_activated)
            self.view.dv_editor_dialog.btn_editconfirm.clicked.connect(self.view.dv_editor_dialog.on_input_change_confirmed)
            self.view.dv_editor_dialog.btn_delete_dv.clicked.connect(self.on_dv_delete_button_clicked)

        # connect signals that are needed in both cases (adding and editing)
        self.view.dv_editor_dialog.lineEdit_nameinput.textChanged.connect(self.view.dv_editor_dialog.check_varname_changed)
        self.view.dv_editor_dialog.lineEdit_searchvariants.textChanged.connect(self.view.dv_editor_dialog.filter_dv_names)
        self.view.dv_editor_dialog.tableWidget_dialog_dvaddvariants.cellDoubleClicked.connect(self.on_edit_dv_change_color_double_clicked)

        dialog_result = self.view.dv_editor_dialog.exec()

        # if the user clicks "save", the changes are saved in the model and the view is updated --> New/Changed formats are added to the table
        if dialog_result == QDialog.DialogCode.Accepted:
            updated_dvs, updated_variants = self.view.dv_editor_dialog.return_temporary_dv_data()
            # send data to model
            ...
            self.view.variable_management_tab.update_detected_variants_table(updated_variants)
    ...
    def on_open_metadata_speaker_editor(self, item: QTreeWidgetItem, column, called_for_add: bool = False):
        """
        ...
        if called_for_add:
            self.view.create_add_speaker_dialog(current_speakers=current_speakers, current_ivs=current_ivs)
            # connect signals that are only needed if the dialog is opened for adding new speakers
            self.view.speaker_editor_dialog.btn_changecolor.clicked.connect(self.on_addspeaker_change_color_clicked)
            self.view.speaker_editor_dialog.lineEdit_nameinput.textChanged.connect(self.view.speaker_editor_dialog.check_varname_changed)
        else:
            ...
            if item is not None:
                item = self.get_tree_item_parent(item)
            self.view.create_edit_speaker_dialog(clicked_item=item, column=column, current_speakers=current_speakers, current_ivs=current_ivs)

            # connect signals that are only needed if the dialog is opened for editing existing speakers
            self.view.speaker_editor_dialog.btn_delete_speaker.clicked.connect(self.on_speaker_delete_button_clicked)
            self.view.speaker_editor_dialog.btn_changecolor.clicked.connect(self.on_editspeaker_change_color_clicked)

        # connect signals that are needed in both cases (adding and editing)
        self.view.speaker_editor_dialog.comboBox_selectspeaker.currentIndexChanged.connect(self.view.speaker_editor_dialog.on_combobox_activated)

        dialog_result = self.view.speaker_editor_dialog.exec()

        if dialog_result == QDialog.DialogCode.Accepted:
            updated_speakers = self.view.speaker_editor_dialog.return_temporary_speaker_data()
    ...
    def on_file_double_clicked(self, row: int, column: int) -> None:
        ...
        if column != 0:
            item: QListWidgetItem = self.view.load_files_tab.list_loaded_filenames.item(row, column)
            file_name = item.text()
    ...
    def on_load_files_finished_clicked(self):
        ...
        if len(self.model.current_project.get_annotation_formats_printable()) > 0: # TODO: Make async
    ...
    def on_annotation_formats_changed(self, annotation_formats:  Dict[str, Tuple[str, str]]) -> None:
        """
        ...
        # Home Tab Anpassung
        self.view.home_menu_tab.update_annotations(len(self.model.current_project.get_detected_annotations()))
    ...
    def on_metadata_dv_changed(self):
        ...
        self.view.variable_management_tab.fill_detected_variants_fully(detected_annos, variants)
        self.view.analysis_settings_tab.add_all_dvs(dvs)
        if self.view.analysis_settings_tab.checkBox_showannotations.isChecked():
            self.view.analysis_settings_tab.highlight_dvs(True, variants, detected_annos) 
        # Home Tab Anpassung
    ...
    def on_metadata_speaker_changed(self):
        ...
        if self.view.analysis_settings_tab.checkBox_showspeakers.isChecked():
            self.view.analysis_settings_tab.underline_speakers(True, speakers, self.model.current_project.get_detected_speakers()) 
        # Home Tab Anpassung
