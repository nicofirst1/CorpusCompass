...
class DatasetCreator():
    ...
    def create_main_dataset(self, ignore_unassigned_dv_values: bool = False) -> pd.DataFrame:
        ...
        temp["variable"] = temp["identifier"].apply(lambda x: self.project.get_parent_name_of_dv_value(x))
        ...
        annotations_with_dvs = temp.loc[~missing_variables, ["identifier", "variable"]].pivot(columns="variable")
        annotations_with_dvs = annotations_with_dvs.droplevel(0, axis=1)
        unassigned_annotations = temp.loc[missing_variables,  ["identifier", "index"]].groupby("index").apply(lambda x: x["identifier"].tolist(), include_groups=False)
        ...
        output_df["speaker"] = detected_annotations.apply(lambda x: self.project.get_speaker_from_text_position(x["file_name"], x["annotation_start"], x["annotation_end"]), axis=1)
        
        # Goal: Add the IV information for each speaker to the output
        temp = pd.DataFrame(index=detected_annotations.index)
        temp["variable_values"] = output_df["speaker"].apply(lambda x: self.project.get_iv_values_from_speaker(x))
        temp = temp.explode("variable_values")
        temp["variables"] = temp["variable_values"].apply(lambda x: self.project.get_parent_name_of_iv_value(x))
        ...
        output_df["file"] = detected_annotations.apply(lambda x: self.project.get_filepath_from_filename(x["file_name"]), axis=1)
